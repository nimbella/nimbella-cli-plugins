...
export default class AuthPostman extends NimBaseCommand {
  ...
  static flags = {
    ...
    delete: flags.string({
      char: 'd',
      description: 'Forget a previously added Postman API key',
    }),
    list: flags.boolean({
      char: 'l',
      description: 'List previously added Postman API keys',
    }),
    show: flags.string({
      description: 'Show the Postman API key associated with given name',
    }),
    current: flags.boolean({
      char: 'c',
      description: 'Show current Postman API key',
    }),
    switch: flags.string({
      char: 's',
      description:
        'Switch to using a particular previously added Postman API key',
    }),
  ...
  async runCommand(
    ...
    if (flagCount > 1) {
      logger.handleError(
        "only one of '--add', '--list', '--switch', '--delete', '--show', or '--current' may be specified",
      )
    } else if (!flags.add && (flags.key || flags.name)) {
      logger.handleError(
        '--name and --key may not be combined with other than --add flag',
      )
    } else if ((flags.key && !flags.name) || (flags.name && !flags.key)) {
      logger.handleError(
        '--name and --key must both be specified if either one is specified',
      )
    }
    if (flags.switch) {
